...
export function LanguageExperience() {
  ...
  function chooseLanguage(next: Language) {
    setLanguage(next);
    setAnswer(null);
    document.getElementById("lesson-preview")?.scrollIntoView({ behavior: reduced ? "instant" : "smooth", block: "center" });
  }

  return <>
    <section className="section-block page-width languages-section" id="languages" aria-labelledby="language-title">
      <div className="section-heading"><p className="eyebrow">CHỌN LỐI BẮT ĐẦU</p><h2 id="language-title">Hôm nay bạn muốn học gì?</h2><p>Thử chọn một ngôn ngữ để xem trước bài học. Bạn có thể đổi bất cứ lúc nào.</p></div>
      <div className="language-grid">
        <button className="language-card language-english" type="button" onClick={() => chooseLanguage("english")} aria-label="Xem trước bài học English"><span className="language-card-top"><Globe2 size={22}/> ENGLISH <ArrowRight size={24}/></span><span className="language-word">Hello<span>!</span></span><span className="language-subtitle">Từ những câu giao tiếp đầu tiên</span><span className="language-card-bottom">Xem bài học mẫu <ArrowRight size={17}/></span></button>
        <button className="language-card language-chinese" type="button" onClick={() => chooseLanguage("chinese")} aria-label="Xem trước bài học Chinese"><span className="language-card-top"><Globe2 size={22}/> 中文 <ArrowRight size={24}/></span><span className="language-word">你好<span>。</span></span><span className="language-subtitle">Từ Pinyin và những chữ đầu tiên</span><span className="language-card-bottom">Xem bài học mẫu <ArrowRight size={17}/></span></button>
      </div>
    </section>
    <section className="section-block page-width preview-section" id="lesson-preview" aria-labelledby="preview-title">
      <div className="section-heading"><p className="eyebrow">NHÌN THỬ MỘT BÀI HỌC</p><h2 id="preview-title">Học thật sẽ trông như thế này.</h2><p>Một câu hỏi nhỏ. Một câu trả lời rõ ràng. Đây chỉ là bản xem trước, chưa lưu tiến độ học.</p></div>
